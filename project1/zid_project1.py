...
def get_tics(pth):
    ...
    for line in file:
        if line is not None:
            line = line.split('=')
            if len(line) >= 2:
                exc = ''.join(c for c in line[0] if c.isalnum())
                tic = ''.join(c for c in line[1] if c.isalnum())
                tics[tic.lower()] = exc.lower()
    return tics
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    data = []
    with open(os.path.join(DATDIR, tic + "_prc.dat"), 'r') as f:
        for line in f:
            data.append(line)
        return data
...
def line_to_dict(line):
    ...
    # Split the line by colWidths
    data = {}
    for i, name in enumerate(COLUMNS):
        data[name] = line[:COLWIDTHS.get(name)]
        line = line[COLWIDTHS.get(name):]
    return data
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        ...
        for tick in tickers_lst:
            if tick not in tic_exchange_dic.keys():
                raise Exception("Tic not in dict")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        tic_data = {}
        tic_data['exchange'] = tic_exchange_dic[tic]
        tic_data['data'] = []
        # Get the lines
        lines = read_dat(tic)
        # Loop through and get the lne dic
        for line in lines:
            line_dict = line_to_dict(line)
            # Remove any columns not in col_lst
            dic_cols = line_dict.keys()
            new_line_dict = {}
            for col in dic_cols:
                if col in col_lst:
                    new_line_dict[col] = line_dict[col]
            # Add to dic_data
            tic_data['data'].append(line_dict)
        data[tic] = tic_data
